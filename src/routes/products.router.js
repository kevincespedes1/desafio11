...
import { isUser } from '../middleware/adminAndUser.js';
...
productsRouter.post('/', uploader.array('files'),isUser, async (req, res) => {
    try {
        const newProduct = {
...
productsRouter.delete('/:pid',isUser, async (req, res) => {
    try {
        const productID = req.params.pid;
        ...
        req.logger.error('Error al eliminar un producto:', err);
